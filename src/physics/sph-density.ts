...
/**
 * SPH density restricted to a contiguous index range — used when the
 * particle system holds two species (DM at [0, dmCount), gas at
 * [gasStart, gasStart + gasCount)) and only one species should
 * contribute to its own density estimate.
 */
export function computeSphDensityForRange(
  ps: ParticleSystem,
  grid: SpatialGrid,
  kernel: SphKernel,
  rangeStart: number,
  ...
  if (densitiesOut.length !== rangeCount) {
    throw new RangeError(
      `computeSphDensityForRange: densitiesOut length ${String(densitiesOut.length)} != rangeCount ${String(rangeCount)}`,
    );
  }
  ...
  for (let g = 0; g < rangeCount; g += 1) {
    const i = rangeStart + g;
    const ii = i * 4;
