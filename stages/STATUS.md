...
_None._ One UX nit: the Vercel project still has Deployment Protection enabled, so the live URL requires SSO to view. User to disable it in the Vercel dashboard (Settings → Deployment Protection → Vercel Authentication: Disabled). Doesn't block Stage 1.
