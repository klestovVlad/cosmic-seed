...
- *react-three-fiber* — rejected for v1 because the simulation loop runs on GPU and we want raw control over render order, transform feedback, and compute pipelines. Reconsider in Stage 5+ if scene complexity demands it.
- *Svelte* — rejected; React 19's compiler + Zustand is enough, and the team familiarity is in React.
- *Cloudflare Pages / Netlify* — viable, but Vercel has the smoothest preview-deploy story for PRs.
